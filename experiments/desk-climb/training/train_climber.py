...
cfg.events['reset_stair_ladder'].params.update(min_start_tread=22,max_start_tread=24,desk_probability=0.)
from mjlab_microduck.tasks import mdp
from mjlab.managers import RewardTermCfg,TerminationTermCfg
cfg.rewards.pop('recovery_progress',None)
cfg.rewards['desk_standing_quality'].weight=0.
cfg.rewards['table_arrival']=RewardTermCfg(func=mdp.floor_desk_supported_arrival_reward,weight=5.)
cfg.terminations['reached_top']=TerminationTermCfg(func=mdp.floor_desk_supported_arrival,time_out=True)
geometry_patch.configure_cfg(cfg)
# Isolate termination height only; identical function and footprint in both arms.
physical_top=float(next(r for r in floor_desk.RECORDS if r['name']=='desktop')['pos'][2])+float(next(r for r in floor_desk.RECORDS if r['name']=='desktop')['size'][2])
assert abs(physical_top-.66)<1e-8
min_z=float(os.environ['RECOVERY_MIN_ROOT_Z']);assert min_z in (.74,.66)
changed=[]
for name,t in cfg.terminations.items():
    if t.func is mdp.floor_desk_recoverable_termination:
        t.func=mdp.floor_desk_recoverable_termination_at_height
        t.params['min_root_z']=min_z;changed.append(name)
assert set(changed)=={'bad_orientation','fallen','off_side','foot_fling','overstep','airborne'},changed
(out/'height-gate-config.json').write_text(json.dumps({'min_root_z':min_z,'physical_tabletop_z':physical_top,'terms':changed,'unchanged_footprint':{'x':[.4,.98],'abs_y':.28},'reward_gates_unchanged':True,'no_actor_inputs':True},indent=2))
...
manifest.update(physical_geometry=geometry_patch.audit(),min_start_tread=22,max_start_tread=24,desk_probability=0.,standing_reward=0.,arrival_reward_weight=5.,allow_table_fall=True)
